...
public class ActiveDisplayView extends FrameLayout {
    ...
    class SettingsObserver extends ContentObserver {
        ...
        public void update() {
            ...
            mDisplayNotifications = Settings.PAC.getInt(
                    resolver, Settings.PAC.ENABLE_ACTIVE_DISPLAY, 0) == 1;
            mDisplayNotificationText = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_TEXT, 0) == 1;
            mShowAllNotifications = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_ALL_NOTIFICATIONS, 0) == 1;
            mHideLowPriorityNotifications = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_HIDE_LOW_PRIORITY_NOTIFICATIONS, 0) == 1;
            mPocketMode = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_POCKET_MODE, POCKET_MODE_OFF);
            mRedisplayTimeout = Settings.PAC.getLong(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_REDISPLAY, 0L);
            mInitialBrightness = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_BRIGHTNESS, 100) / 100f;
            mSunlightModeEnabled = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_SUNLIGHT_MODE, 0) == 1;
            String excludedApps = Settings.PAC.getString(resolver,
                    Settings.PAC.ACTIVE_DISPLAY_EXCLUDED_APPS);
            mDisplayTimeout = Settings.PAC.getLong(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_TIMEOUT, 8000L);
            mTurnOffModeEnabled = Settings.PAC.getInt(
                    resolver, Settings.PAC.ACTIVE_DISPLAY_TURNOFF_MODE, 0) == 1;
            mProximityThreshold = Settings.PAC.getLong(
            ...
            int brightnessMode = Settings.PAC.getInt(
                    resolver, Settings.PAC.SCREEN_BRIGHTNESS_MODE, -1);
    ...
    private void setBrightness(float brightness) {
        ...
        mBrightnessMode = Settings.PAC.getInt(resolver, Settings.PAC.SCREEN_BRIGHTNESS_MODE,
                Settings.PAC.SCREEN_BRIGHTNESS_MODE_AUTOMATIC);
        if (mBrightnessMode != Settings.PAC.SCREEN_BRIGHTNESS_MODE_AUTOMATIC) {
            mUserBrightnessLevel = Settings.PAC.getInt(resolver, Settings.PAC.SCREEN_BRIGHTNESS,
            ...
            Settings.PAC.putInt(resolver,
                    ...
                    Settings.PAC.SCREEN_BRIGHTNESS_MODE_MANUAL);
            try {
