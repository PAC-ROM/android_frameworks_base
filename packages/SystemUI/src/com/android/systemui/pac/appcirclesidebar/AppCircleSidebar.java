    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.ENABLE_APP_CIRCLE_BAR), false, this);
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.WHITELIST_APP_CIRCLE_BAR), false, this);
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.APP_CIRCLE_BAR_TRIGGER_WIDTH), false, this);
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.APP_CIRCLE_BAR_TRIGGER_TOP), false, this);
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.APP_CIRCLE_BAR_TRIGGER_HEIGHT), false, this);
            resolver.registerContentObserver(Settings.PAC.getUriFor(
                    Settings.PAC.APP_CIRCLE_BAR_SHOW_TRIGGER), false, this);
            update();
        }
        ...
        public void update() {
            final ContentResolver resolver = mContext.getContentResolver();
            setAppBarVisibility(Settings.PAC.getIntForUser(
                    resolver, Settings.PAC.ENABLE_APP_CIRCLE_BAR, 0,
                    UserHandle.USER_CURRENT_OR_SELF) == 1);
            String includedApps = Settings.PAC.getStringForUser(resolver,
            ...
            int width = Settings.PAC.getInt(
                    resolver, Settings.PAC.APP_CIRCLE_BAR_TRIGGER_WIDTH, 10);
            if (mTriggerWidth != width)
                setTriggerWidth(width);
            setTopPercentage(Settings.PAC.getInt(
                    resolver, Settings.PAC.APP_CIRCLE_BAR_TRIGGER_TOP, 0) / 100f);
            setBottomPercentage(Settings.PAC.getInt(
                    resolver, Settings.PAC.APP_CIRCLE_BAR_TRIGGER_HEIGHT, 100) / 100f);
            if (Settings.PAC.getInt(
                    resolver, Settings.PAC.APP_CIRCLE_BAR_SHOW_TRIGGER, 0) == 1)
                showTriggerRegion();
            else
                hideTriggerRegion();
        }
