    ...
    public void onScreenTurnedOff(int why) {
        synchronized (this) {
            ...
            if (mExitSecureCallback != null) {
                ...
                final ContentResolver crAOKP = mContext.getContentResolver();
                // user turned the screen off likely
                // we wanna check if the user wants to use the timeout setting here too
                boolean userOverride = Settings.Secure.getInt(crAOKP,
                        Settings.Secure.LOCK_SCREEN_LOCK_USER_OVERRIDE, 0) == 1;

                if (userOverride) {

                    // From DisplaySettings
                    long displayTimeout = Settings.System.getInt(crAOKP, SCREEN_OFF_TIMEOUT,
                            KEYGUARD_DISPLAY_TIMEOUT_DELAY_DEFAULT);

                    // From SecuritySettings
                    final long lockAfterTimeout = Settings.Secure.getInt(crAOKP,
                            Settings.Secure.LOCK_SCREEN_LOCK_AFTER_TIMEOUT,
                            KEYGUARD_LOCK_AFTER_DELAY_DEFAULT);
